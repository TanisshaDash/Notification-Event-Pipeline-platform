...
export function FireEvent({ token, onFired }: { token: string; onFired: () => void }) {
  ...
  return (
    <div style={{ border: "1px solid var(--border)", padding: 20, background: "var(--bg2)", animation: "fadeIn .6s ease both" }}>
      ...
      <textarea value={payload} onChange={e => setPayload(e.target.value)} rows={7}
      ...
      <button onClick={fire} disabled={status === "sending"} style={{
        ...
        background: status === "ok" ? "var(--green)" : status === "err" ? "var(--red)" : "var(--text)",
